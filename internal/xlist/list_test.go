...
func checkListPointers[T comparable](t *testing.T, l List[T], es []*Element[T]) {
	...
	if len(es) == 0 {
		if l.(*list[T]).root.next != nil && l.(*list[T]).root.next != root || l.(*list[T]).root.prev != nil && l.(*list[T]).root.prev != root {
			t.Errorf("l.root.next = %p, l.root.prev = %p; both should both be nil or %p", l.(*list[T]).root.next, l.(*list[T]).root.prev, root)
		}
